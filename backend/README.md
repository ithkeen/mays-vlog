...
└── app/
    ...
    ├── api/            # 业务路由（T7：tasks.py，挂载于 /api 前缀）
    │   └── tasks.py    # POST/GET/PATCH/DELETE /api/tasks(...)
...
## HTTP API 路由 `app/api/tasks.py`

T7 把编排器与持久层封装成 6 个 JSON 端点，全部挂载在 `/api` 前缀下
（`main.py: app.include_router(tasks_router, prefix="/api")`）。请求 / 响应
模型用 pydantic v2 BaseModel；`prompt` 字段 `min_length=1, max_length=2500`。

### 错误响应约定

- 业务错误统一 `{"error": "<code>", ...optional}`：
  - `task_not_found` / `task_not_playable` / `task_in_progress` / `image_decode_failed`
- 字段级校验失败（prompt 空 / 超长、JSON 类型错误等）由 FastAPI 默认产生 422
  `{"detail": [...]}`，**保留默认体不改写**——前端可按 422 统一处理
- `DELETE` 时若 UFile 对象删除失败：SQLite 行已删，UFile 残留视为可后续手动
  清理的孤儿，路由层 log warning 后**仍返回 204**（前端视角的删除已生效）

### 端点示例

#### 1. POST `/api/tasks` — 提交新任务
...
curl -X POST http://localhost:8000/api/tasks \
  ...
  -d '{"prompt": "A cinematic shot of a futuristic city at dusk", "image": null}'
```

```json
{ "prompt": "A cinematic shot of a futuristic city at dusk", "image": null }
```

> `image`：raw base64 字符串（**不带** `data:image/...;base64,` 前缀），`null` 表示文生。

成功 200：

```json
{ "id": "8f6a3b1c-...", "status": "pending" }
```

并发冲突 409：

```json
{ "error": "task_in_progress", "current_task_id": "<当前正在跑的 id>" }
```
...
prompt 空 / 超长 / JSON 字段类型错 → 422（FastAPI 默认体）：

```json
{ "detail": [{ "type": "string_too_short", "loc": ["body", "prompt"], "msg": "...", "ctx": {"min_length": 1} }] }
```

#### 2. GET `/api/tasks` — 历史列表（仅 success，按 finished_at DESC）
...
[
  {
    ...
    "finished_at": 1778517123
  }
]
```

> 时间字段为 **unix 秒**（int）；`has_image` 为 bool（DB 存 0/1，路由层转换）。

#### 3. GET `/api/tasks/{id}` — 单条详情（任意状态）

```bash
curl http://localhost:8000/api/tasks/8f6a3b1c-...
```
...
#### 4. GET `/api/tasks/{id}/play_url` — 签发 1 小时预签名 URL

仅 `status=success` 返回 200；不存在 / 非 success 一律 404。

```bash
curl http://localhost:8000/api/tasks/8f6a3b1c-.../play_url
```
...
```json
{
  "url": "https://<bucket>.cn-bj.ufileos.com/videos/8f6a3b1c-....mp4?Expires=...&Signature=...",
  "expires_in": 3600
}
```

不可播放（不存在 / 还在跑 / failure）404：

```json
{ "error": "task_not_playable" }
```

#### 5. PATCH `/api/tasks/{id}` — 重命名标题

```bash
curl -X PATCH http://localhost:8000/api/tasks/8f6a3b1c-... \
  -H 'Content-Type: application/json' \
  -d '{"title": "我的城市夜景"}'
```
...
```json
{ "id": "8f6a3b1c-...", "title": "我的城市夜景" }
```

不存在 404：`{"error": "task_not_found"}`。

#### 6. DELETE `/api/tasks/{id}` — 删除任务行 + 视频对象

```bash
curl -X DELETE http://localhost:8000/api/tasks/8f6a3b1c-...
```

成功 204（无响应体）。若该行有 `ufile_object_key`，会同时调
`ufile.delete_object` 清理 UFile 对象；UFile 删除失败仅 log warning，
不影响 204 返回（详见上方「错误响应约定」）。

不存在 404：`{"error": "task_not_found"}`。

### 交互式调试

uvicorn 启动后访问 `http://localhost:8000/docs` 查看 OpenAPI Swagger UI，
可直接在浏览器内构造请求测试这 6 个端点。
